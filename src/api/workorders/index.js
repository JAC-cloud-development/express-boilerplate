...
router.get("/", validateJWT, async function (request, response, next) {
  try {
    return response.json(await Workorders.find().populate('userId'));
  } catch (e) {
    next(e)
  }
});
...
router.get("/:id", validateJWT, async function (request, response, next) {
  try {
    const element = await Workorders.findOne({ _id: request.params.id });
...
router.post("/", validateJWT, async function (request, response, next) {
  try {
    return response.json(await Workorders.create(request.body));
  } catch (e) {
    next(e)
  }
});
...
router.delete("/:id", validateJWT, async function (request, response, next) {
  try {
    const result = await Workorders.deleteOne({ _id: request.params.id });
